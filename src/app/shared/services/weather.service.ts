...
export class WeatherService implements OnDestroy {
  ...
  getCityWeatherByQuery(query: string): Observable<CityWeather> {
    const params = new HttpParams({ formObject: { q: query } });
...
getCityWeatherByCoord(lat: number, lon: number): Observable<CityWeather> {
  const params = new HttpParams({formObject: {
      lat: lat.toString(),
      lon: lon.toString(),
    }});
...
getCityWeatherDetails(lat: number, lon: number): Observable<CityDailyWeather> {
  const params = new HttpParams({formObject: {
  lat: lat.toString(),
  lon: lon.toString,
...
private doGet<T>(url: string, params: HttpParams): Observable < T > {
  params = HttpParams.append('appid', environment.apiKey)
  params = HttpParams.append('lang', 'pt_br');
